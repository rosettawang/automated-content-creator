...
17. **Edit chat falsely retracts real audio changes** (observed 2026-08-03, edit 33). Asked to "add audio from made up script", the chat correctly set `audio_mode='voiceover'` + a 262-char `vo_script` — which export really does synthesize via `tts.synthesize()`. But when the user asked "did you really add the voiceover, what model?", it denied the work ("I'm an editing assistant, not a voice generator") and claimed the timeline was "exactly as-is". Both wrong, and the second answer misleads the user about the state of their own cut. Cause: `revise_edit`'s prompt tells the model to *set* `audio_plan` but never says what happens downstream, so under challenge it reasons from its own text-only nature rather than the pipeline's. Fix: state in the prompt that setting `audio_plan` is a real change rendered at export by TTS (and that a follow-up question is not evidence of error); pair with a visible audio-plan indicator on the cut so the state is legible without asking the chat.
